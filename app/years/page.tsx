  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-10">
      ...
      <p className="text-[var(--color-ink-muted)] mb-8 leading-relaxed">
        ...
        (crowdsourced) ตอนนี้มีแค่ 2569 ที่ข้อมูลครบที่สุด ส่วนปีอื่นๆ
        จะค่อยๆ เพิ่มเมื่อเปิดให้รุ่นพี่กรอกย้อนหลังในเฟสถัดไป
      </p>
            ...
            <div className="flex items-baseline justify-between gap-4 flex-wrap">
              <div>
                <div className="flex items-baseline gap-3 mb-1">
                  <span className="font-serif text-3xl font-semibold !text-[var(--color-ink)]">
                    {y.id}
                  </span>
                  <span className="text-sm !text-[var(--color-ink-muted)]">{y.name}</span>
                </div>
                {y.notes && (
                  <p className="text-sm !text-[var(--color-ink-muted)] mt-2 max-w-xl">{y.notes}</p>
                )}
              </div>
      ...
      <div className="mt-10 bg-[var(--color-accent-soft)]/30 border border-[var(--color-border)] rounded-xl p-5">
        ...
        <p className="text-sm text-[var(--color-ink-muted)] leading-relaxed">
          ...
          เรากำลังเปิด crowdsource เพื่อให้ข้อมูลเหล่านี้กลับมามีชีวิตอีกครั้ง
          ตอนนี้บอกเพื่อนรุ่นพี่ให้รอติดตาม
        </p>
      </div>
    </div>
  );
